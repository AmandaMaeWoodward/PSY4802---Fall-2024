...
```{r}
ggplot(penguinPolarBearEmotion, aes(emotionsOfPenguins, meanPenguinsBorn, fill=polarbearPresence))+ geom_bar(stat="identity", position= position_dodge())
```

Alternatively, could use facet wrap with boxplots
```{r}
ggplot(penguinData, aes(emotionsOfPenguins, penguinsBorn, fill= polarbearPresence))+ geom_boxplot()+ facet_wrap(~polarbearPresence)
```
...
```{r}
predict2<- ggpredict(lm2, terms= c("fishAvailable","polarbearPresence"))
plot(predict2)
```
