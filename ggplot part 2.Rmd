...
library(tidyverse)
data(diamonds)
ggplot(diamonds, aes(carat, price, color=cut))+ geom_point()+ theme(legend.position="none")
```

We can also change the location: 
```{r}
ggplot(diamonds, aes(carat, price, color=cut))+ geom_point()+ theme(legend.position="bottom")
```
...
library(viridis)
data(mtcars)
ggplot(mtcars, aes(disp, hp, color= mpg))+ geom_point()+ scale_color_viridis(discrete=FALSE, option="inferno")+ theme_classic() 
```

2) Move the legend to the top of the graph
```{r}
ggplot(mtcars, aes(disp, hp, color= mpg))+ geom_point()+ scale_color_viridis(discrete=FALSE, option="inferno")+ theme_classic()+ theme(legend.position="top") 

ggplot(mtcars, aes(disp, hp, color= mpg))+ geom_point()+ scale_color_viridis(discrete=FALSE, option="inferno")+ theme_classic()+ theme(legend.position="bottom")
```

3) Remove the legend from the graph 
```{r}
ggplot(mtcars, aes(disp, hp, color= mpg))+ geom_point()+ scale_color_viridis(discrete=FALSE, option="inferno")+ theme_classic()+ theme(legend.position="none")
```

4) Change the title of the legend
```{r}
ggplot(mtcars, aes(disp, hp, color= mpg))+ geom_point()+ scale_color_viridis(discrete=FALSE, option="inferno", name= "Miles Per  Gallon")+ theme_classic()+ theme(legend.position="top")
```
...
ggplot(mtcars, aes(mpg))+ geom_histogram(bins= 5, fill= "blue") + facet_wrap(~cyl)+ylab(label = "frequency") + xlab(label= "miles per gallon")
...
data(mtcars)
ggplot(mtcars, aes(mpg))+ geom_histogram(bins= 5, fill= "blue")+ facet_wrap(~cyl, labeller= as_labeller(labels))
```

### Practice: 
1) create a facet wrap graph to generate a scatterplot of mpg and hp based on the number of cylinders a car has
```{r}
prettyGraph <- ggplot(mtcars, aes(mpg, hp, color=as.factor(cyl)))+ geom_point()+ facet_wrap(~cyl, labeller= as_labeller(labels))+ theme_classic()+ ylab(label="Horse Power")+ xlab(label= "Miles per Gallon")+ scale_color_viridis(discrete=TRUE, option = "D")

finalPrettyGraph<-prettyGraph + theme(legend.position= "none", text = 
...
```{r}
ggsave("PSY4802-FinalPrettyGraph.tiff", width= 16, height= 10, units = "in", dpi= 300)
