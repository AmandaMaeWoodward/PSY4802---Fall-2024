...
```{r}
ChickLong<- pivot_longer(ChickWide, cols = c(colnames(ChickWide)[3:14]), names_to="Day", values_to= "Weight")
```
...
```{r}
ChickLong1<- ChickWide %>%  pivot_longer(cols = c(colnames(ChickWide)[3:14]), names_to="Day", values_to= "Weight")
```

```{r}
ChickLong1<- ChickWide %>%  
  pivot_longer(cols = c(colnames(ChickWide)[3:14]), names_to="Day", values_to= "Weight") %>% 
  filter(Chick==1)
```
...
```{r}
subjNo<- aggressionData$subjNo
anxietyScore<- sample(x = 1:20,size = nrow(aggressionData), replace= TRUE) 
# x is what you're sampling from
# size is # of things to pull
# replace is true
...
```{r}
library(dplyr)
finaldata<- left_join(aggressionData, AnxietyData)
```
...
```{r}
aggressionData$parentReportGender
```
...
```{r}
aggressionData$parentReportGender<- as.factor(aggressionData$parentReportGender)

levels(aggressionData$parentReportGender)<-c("girl", "boy")

aggressionData$parentReportGender<- factor(aggressionData$parentReportGender, levels= c("boy","girl"))
```
...
```{r}
aggressionData$parentReportGender<- relevel(aggressionData$parentReportGender, ref= "girl") 
```
...
```{r}
aggressionData$attention<- rep(1:6,8)
```
...
```{r}
library(car)
aggressionData$attention<- car::recode(aggressionData$attention, "6=1;5=2;4=3;3=4;2=5;1=6")
```
...
```{r}
aggressionData %>% separate(subjNo, into = c("condition2", "subj"), sep=1)
```
